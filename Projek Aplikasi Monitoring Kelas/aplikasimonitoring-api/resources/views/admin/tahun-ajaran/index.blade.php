...
@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="p-6 border-b border-gray-100">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Total: {{ $tahunAjarans->total() }}</p>
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('admin.tahun-ajaran.create') }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg flex items-center"><i class="fas fa-plus mr-2"></i>Tambah Tahun Ajaran</a>
                </div>
            </div>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase">Tahun</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($tahunAjarans as $index => $ta)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm text-gray-500">{{ $tahunAjarans->firstItem() + $index }}</td>
                            <td class="px-6 py-4 font-semibold text-gray-800">{{ $ta->tahun }}</td>
                            <td class="px-6 py-4">
                                @if($ta->flag)
                                    <span class="px-3 py-1 text-xs rounded-full font-medium bg-green-100 text-green-700">Aktif</span>
                                @else
                                    <span class="px-3 py-1 text-xs rounded-full font-medium bg-gray-100 text-gray-600">Tidak Aktif</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center space-x-2">
                                    <a href="{{ route('admin.tahun-ajaran.edit', $ta) }}" class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><i class="fas fa-edit"></i></a>
                                    <form action="{{ route('admin.tahun-ajaran.destroy', $ta) }}" method="POST" class="inline" onsubmit="return confirm('Yakin?')">
                                        @csrf @method('DELETE')
        ...
        @if($tahunAjarans->hasPages())<div class="p-6 border-t border-gray-100">{{ $tahunAjarans->links() }}</div>@endif
    </div>
    
    <div id="importModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
            <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800">Import Tahun Ajaran</h3>
            ...
            <form action="{{ route('admin.tahun-ajaran.import') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                <div class="mb-4">
                ...
                <div class="mb-4 p-4 bg-blue-50 rounded-lg"><p class="text-sm text-blue-700"><i class="fas fa-info-circle mr-1"></i>Kolom: tahun, flag/aktif</p></div>
