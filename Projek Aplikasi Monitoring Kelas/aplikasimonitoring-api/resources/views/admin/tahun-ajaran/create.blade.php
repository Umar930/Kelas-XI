...
@section('content')
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-6 border-b border-gray-100"><h3 class="text-lg font-semibold text-gray-800">Form Tambah Tahun Ajaran</h3></div>
            <form action="{{ route('admin.tahun-ajaran.store') }}" method="POST" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="tahun" value="{{ old('tahun') }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('tahun') border-red-500 @enderror" placeholder="2024/2025">
                    @error('tahun')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="flag" value="1" {{ old('flag') ? 'checked' : '' }} class="w-4 h-4 text-indigo-500 border-gray-300 rounded focus:ring-indigo-500">
