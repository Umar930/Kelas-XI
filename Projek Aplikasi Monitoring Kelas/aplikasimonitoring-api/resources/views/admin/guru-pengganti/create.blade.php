...
@section('page-title', 'Tambah Guru Pengganti')
@section('page-subtitle', 'Buat data guru pengganti baru')

@section('content')
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-6 border-b border-gray-100">
                <h3 class="text-lg font-semibold text-gray-800">Form Tambah Guru Pengganti</h3>
            </div>
            <form action="{{ route('admin.guru-pengganti.store') }}" method="POST" class="p-6">
                @csrf
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal <span class="text-red-500">*</span></label>
                    <input type="date" name="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('tanggal') border-red-500 @enderror">
                ...
                <div class="mb-6">
                    ...
                    <select name="guru_pengganti_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('guru_pengganti_id') border-red-500 @enderror">
                        ...
                        @foreach($gurus as $g)
                            <option value="{{ $g->id }}" {{ old('guru_pengganti_id') == $g->id ? 'selected' : '' }}>{{ $g->guru }} - {{ $g->kode_guru }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="time" name="jam_mulai" value="{{ old('jam_mulai') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Jam Selesai</label>
                        <input type="time" name="jam_selesai" value="{{ old('jam_selesai') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Catatan</label>
                    <textarea name="catatan" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Catatan...">{{ old('catatan') }}</textarea>
                </div>
