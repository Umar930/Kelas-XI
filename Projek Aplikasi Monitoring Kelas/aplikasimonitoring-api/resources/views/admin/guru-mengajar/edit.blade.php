...
@section('page-title', 'Edit Guru Mengajar')
@section('page-subtitle', 'Ubah data guru mengajar')

@section('content')
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-6 border-b border-gray-100">
                <h3 class="text-lg font-semibold text-gray-800">Form Edit Guru Mengajar</h3>
            </div>
            <form action="{{ route('admin.guru-mengajar.update', $guruMengajar) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                <div class="mb-6">
                    ...
                    <select name="guru_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('guru_id') border-red-500 @enderror">
                        ...
                        @foreach($guruList as $g)
                            <option value="{{ $g->id }}" {{ old('guru_id', $guruMengajar->guru_id) == $g->id ? 'selected' : '' }}>{{ $g->guru }} - {{ $g->kode_guru }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="mapel_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('mapel_id') border-red-500 @enderror">
                        ...
                        @foreach($mapelList as $m)
                            <option value="{{ $m->id }}" {{ old('mapel_id', $guruMengajar->mapel_id) == $m->id ? 'selected' : '' }}>{{ $m->mapel }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="kelas_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('kelas_id') border-red-500 @enderror">
                        ...
                        @foreach($kelasList as $k)
                            <option value="{{ $k->id }}" {{ old('kelas_id', $guruMengajar->kelas_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kelas }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="tahun_ajaran_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 @error('tahun_ajaran_id') border-red-500 @enderror">
                        ...
                        @foreach($tahunAjaranList as $t)
                            <option value="{{ $t->id }}" {{ old('tahun_ajaran_id', $guruMengajar->tahun_ajaran_id) == $t->id ? 'selected' : '' }}>{{ $t->tahun }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        <option value="aktif" {{ old('status', $guruMengajar->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="non-aktif" {{ old('status', $guruMengajar->status) == 'non-aktif' ? 'selected' : '' }}>Non-Aktif</option>
                    </select>
                </div>
