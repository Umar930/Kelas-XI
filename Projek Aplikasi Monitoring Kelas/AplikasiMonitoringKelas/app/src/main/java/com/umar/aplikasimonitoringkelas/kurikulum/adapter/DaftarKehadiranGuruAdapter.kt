...
/**
 * Adapter untuk menampilkan daftar kehadiran guru (read-only)
 */
class DaftarKehadiranGuruAdapter(
    private val jadwalList: List<JadwalItem>
) : RecyclerView.Adapter<DaftarKehadiranGuruAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        // Calculate jam ke from index or use jamKe field
        val jamKe = if (item.jamKe > 0) item.jamKe else (position + 1)
        holder.tvJamKe.text = jamKe.toString()
        holder.tvNamaGuru.text = item.guruNama ?: item.namaGuru ?: item.guru ?: "Unknown"
        holder.tvNamaMapel.text = item.mapelNama ?: item.namaMapel ?: item.mapel ?: "Unknown"
        ...
        val guruPenggantiName = item.guruPenggantiNama
        val hasRequestPengganti = item.requestPengganti == true
        ...
        } else if (hasRequestPengganti) {
            holder.tvGuruPengganti.text = "Request Pengganti"
            holder.tvGuruPengganti.setTextColor(ContextCompat.getColor(context, R.color.status_izin))
            holder.tvGuruPengganti.visibility = View.VISIBLE
        } else {
